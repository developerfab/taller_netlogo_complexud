...
to crear-tortugas
  ca
  crt numero-tortugas ;; Toma el valor de la interfaz (deslizador)
  ask turtles [
    set label who ;; Para saber qué tortuga es, muestra el númeor del label encima de cada tortuga en el mundo
    set shape "circle"
    set size 2
    set color seleccion-color ;; Toma el valor de la interfaz
  ]
end
...
to mover
  ask turtles [
    ...
    ;; Controlando el movimiento con los límites del mundo
    ;; xcor<16 y xcor>-16
    ;; ycor<16 y ycor>-16
    if (xcor > 15 or xcor < -15  or ycor > 15 or ycor < -15) [
      left random 180
      if (azar-color = true) [ ;; Toma el valor de la interfaz
        set color random 140
      ]
      ;; Hacemos que en el choque suene
      sound:play-note "Violin" (xcor + 6) 64 2 ;; 64 es la velocidad con la que va el instrumento, el 2 es la duración
      fd 1
    ]
  ]
end
